...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 h-16 z-50 bg-[#F7FAFC] dark:bg-gray-900 border-b-2 border-gray-200 dark:border-gray-700">
      ...
      {isMobileMenuOpen && (
        <>
          ...
          />
          <div className="fixed top-16 left-0 right-0 bg-[#F7FAFC] dark:bg-gray-900 border-b-2 border-gray-200 dark:border-gray-700 z-40 lg:hidden">
            <div className="px-4 py-4 space-y-2">
              {navItems.map((item) => (
                <a
                  ...
                  href={item.href}
                  onClick={(e) => handleScroll(e, item.href)}
                  className="block text-gray-600 dark:text-gray-300 px-4 py-3 rounded-lg text-base font-medium hover:bg-gray-200 dark:hover:bg-gray-800 transition-colors"
                >
                  {item.label}
                </a>
              ))}
              <div className="pt-4 space-y-2">
                <Link
                  href="/sign-in"
                  className="block text-center text-gray-600 dark:text-gray-300 px-4 py-3 rounded-xl border border-gray-600 dark:border-gray-400 text-sm hover:bg-gray-200 dark:hover:bg-gray-800 transition-colors"
                >
                  Login
                </Link>
                <Link
                  href="/sign-up"
                  className="block text-center px-4 py-3 rounded-xl bg-gray-600 dark:bg-gray-700 text-white text-sm hover:bg-gray-700 dark:hover:bg-gray-600 transition-colors"
                >
                  Mulai Sekarang
                </Link>
              </div>
